...
SELECT SUM(surfacearea) AS continental_surface_area
FROM country
GROUP BY continent
...
SELECT (gnp - gnpold) AS gnp_difference
    , ABS(gnp - gnpold) AS gnp_abs_difference
FROM country
WHERE NOT (gnp IS NULL)
    AND NOT (gnpold IS NULL)
...
SELECT COUNT(name) AS num_countries_in_continent
FROM country
GROUP BY continent
ORDER BY num_countries_in_continent DESC;
...
SELECT MAX(population) AS pop_max
FROM city
GROUP BY countrycode
