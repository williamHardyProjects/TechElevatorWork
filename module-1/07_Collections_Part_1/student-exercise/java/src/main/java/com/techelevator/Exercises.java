...
public class Exercises 
{
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) 
	{
		...
		{
			intList.add((double) intArray[i] / 2);
		}
		return intList;
	}
	...
	{
		int count = 0;
		for (int intItem : integerList) 
		{
			if (intItem == intToFind) 
			{
				count++;
			}
		}
	...
	public List<String> fizzBuzzList(Integer[] integerArray) 
	{
		...
		for (int intItem : integerArray) 
		{
			...
			if (fizzBuzz.equals("")) 
			{
				strList.add(Integer.toString(intItem));
			} 
			else 
			{
